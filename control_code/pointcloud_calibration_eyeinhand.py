...
calibration_xyzrpy = [0.0351681, 0.0485623, -0.154518, -0.00180744, 0.148223, -3.13968]
...
class MoveIt_Control:
            ...
            try:
                ...
                for target_xyz in all_target_xyz:
                    for xyz in target_xyz:
                        ...
                        target_pos.append([xyz[0],xyz[1],xyz[2], 2.771903177707273, 0.06450328204639298, 1.9958948635653042])
                        # 0.21353610528730255, 0.4218265785417495, 0.9750384195381976, -3.0473277581834797, 0.6593488993345046, -2.4849324234603474
                    ...
                    self.move_l(target_pos, waypoints_number=len(target_xyz), speed_scale = 0.5)
                    rospy.sleep(2)
    ...
    def calibration(self):
        ...
        self.camera2tool_pose_matrix = concatenate_matrices(trans_matrix,rot_matrix)  # 与np.dot(trans_matrix, qua_matrix)等价

        xyzrpy = [xyz[0],xyz[1],xyz[2], rpy[0],rpy[1],rpy[2]]

        print(self.camera2tool_pose_matrix)
        print(xyzrpy)

        return self.camera2tool_pose_matrix, xyzrpy
    ...
    def get_endpos(self):
        ...
        rot_matrix = quaternion_matrix([qx,qy,qz,qw])
        ee2base_pose_matrix = concatenate_matrices(trans_matrix,rot_matrix)  # 与np.dot(trans_matrix, qua_matrix)等价
        return ee2base_pose_matrix
    ...
    def camera_detect(self):
        ...
        ee2base_pose_matrix = self.get_endpos()
        print("get_endpos")
        for pos in trajectory_points:
            if pos[0]!=0:
                trans_matrix = translation_matrix(pos)
                object_matrix = np.dot(self.camera2tool_pose_matrix, trans_matrix)
                object_matrix = np.dot(ee2base_pose_matrix, object_matrix)
    ...
    def prepare(self,a=1,v=1):
        self.move_j([0.3091201604655991, 0.3426448531299066, 1.0437498267643968, 2.5826462426619994, 0.01813936357117019, 1.9866596565567258],a=a,v=v)
...
if __name__ =="__main__":
    ...
    add_thread2 = threading.Thread(target = publish_calibration_tf,args=["eeffector","camera_base"])
    add_thread2.start()
